...
if "similarity_matrix" not in st.session_state:
    ...
    st.session_state.similarity_ready = False

def compute_similarity():
    # Combine features
    features = (movies["genres"] + " " + movies["keywords"] + " " +
                movies["tagline"].fillna("") + " " + movies["cast"] + " " + movies["director"])
    ...
    st.session_state.similarity_ready = True

# Start background thread if not already computed
if not os.path.exists("data/similarity.npy") and not st.session_state.similarity_ready:
    st.info("⏳ Computing movie similarity in the background…")
    threading.Thread(target=compute_similarity).start()
elif os.path.exists("data/similarity.npy"):
    st.session_state.similarity_matrix = np.load("data/similarity.npy")
    st.session_state.similarity_ready = True

# --- Recommendation Logic ---
def recommend_by_movie(movie_name, top_n=12):
    if st.session_state.similarity_matrix is None:
        st.warning("Movie similarity not ready yet. Showing trending movies instead.")
        return movies.sort_values("vote_average", ascending=False).head(top_n)
    ...
    scores = list(enumerate(st.session_state.similarity_matrix[idx]))
...
if st.session_state.selected_movie:
    ...
    show_row("Similar Movies", recommend_by_movie(base_movie, 10))
    ...
    show_row("Search Results", recommend_by_movie(movie_input, 12))

elif actor_input:
    show_row(f"Movies Featuring {actor_input.title()}", filter_movies(actor=actor_input, limit=12))

elif genre_input:
    show_row(f"{genre_input.title()} Movies", filter_movies(genre=genre_input, limit=12))
